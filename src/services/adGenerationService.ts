...
const buildComposePrompt = (params: {
  ...
  lines.push(
    "원본 인물의 얼굴은 첫 번째 이미지에서 픽셀 단위로 그대로 보존해줘 — 재생성·스타일링·매끈하게 만들기·표정 변경 금지.",
    "이 얼굴은 다음 단계(Stage 3 face-swap)에서 카탈로그 모델로 교체되는 placeholder이므로 정면 가까운 각도(최대 20° 회전), 균일 조명, 무표정 또는 가벼운 미소, 매끈한 피부, 무수염 상태를 유지해야 함.",
